...
class KeyQL {
	static validateKeys(dataset, mapFunction) {
		return dataset.length ? Object.keys(mapFunction(dataset[0])) : [];
	}

	static validateRows(dataset, mapFunction, keys) {
		return dataset.map((row, i) => {
			row = mapFunction(row);
			let refRow = Object.defineProperty(Object.create(null), '__keyqlid__', {
				value: i
			});
	...
	constructor(dataset = [], mapFunction = v => v) {
		...
		this._keys = this.constructor.validateKeys(
			this._dataset,
			this._mapFunction
		);
		this._updated = {};
	...
	__initialize__() {
		this._rows = this.constructor.validateRows(
			this._dataset,
			this._mapFunction,
			this._keys
		);
